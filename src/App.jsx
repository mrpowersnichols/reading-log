...
const ABANDON_REASONS = ["Too hard", "Not interested", "Wrong genre for me right now", "Other (write your own)"];
...
export default function ReadingLogApp() {
  ...
  const reviewCountsByTitle = useMemo(() => {
    const counts = {};
    readingLog.forEach((e) => {
      if (e.status === "finished" && e.review) counts[e.title] = (counts[e.title] || 0) + 1;
    });
    return counts;
  }, [readingLog]);
  ...
  function getReviewsForTitle(title) {
    return readingLog
      .filter((e) => e.title === title && e.status === "finished" && e.review)
  ...
  async function addLibraryBookQuiet({ title, author, genre, isbn, cover, copies: n }) {
    await fsAddLibraryBook(CLASS_ID, { title, author, genre, isbn, cover, copies: n });
  }
  ...
  useEffect(() => {
    if (view === "teacher" && !isTeacher) setView("log");
  }, [view, isTeacher]);
    ...
    <div className="rl-root">
      ...
      <div className="rl-shell">
        ...
        <nav className="rl-shelf">
          ...
          <button className={`rl-spine ${view === "status" ? "active" : ""}`} onClick={() => setView("status")}><TrendingUp /> Reading Status</button>
                      ...
                      <div className="rl-card-body">
                        ...
                        <div className="rl-card-text">
                          ...
                          {reviewCountsByTitle[b.title] > 0 && (
                            <button className="rl-btn small" style={{ marginTop: 6 }} onClick={() => setReviewsFor(b.title)}>
          ...
          <div style={{ maxHeight: 360, overflowY: "auto" }}>
            {reviews.map((r) => (
              <div className="rl-review-item" key={r.id}>
                ...
                <div className="rl-review" style={{ borderTop: "none", paddingTop: 4 }}>&ldquo;{r.review}&rdquo;</div>
              </div>
            ))}
          </div>
        )}
...
function ReadingStatusPanel({ uid, myName, isTeacher, students, readingLog, statusEntries, onAdd }) {
  ...
  Object.values(byLogEntry).forEach((arr) => arr.sort((a, b) => (a.date < b.date ? -1 : 1)));
...
function EditEntryModal({ book, onClose, onSave }) {
  ...
  function handleSave() {
    ...
    if (book.status === "abandoned") { updates.dateAbandoned = dateAbandoned; updates.reason = isOther ? customReason.trim() : reason; }
    onSave(updates);
  }
...
function CloseBookModal({ mode, onClose, onSave }) {
  ...
  const isOther = reason === ABANDON_OTHER;
  const canSave = isFinish
    ? rating > 0 && review.trim().length > 0
    : !isOther || customReason.trim().length > 0;

  function handleSave() {
    onSave({ date, rating, review, reason: isOther ? customReason.trim() : reason });
  }
  ...
  return (
    <div className="rl-overlay" onClick={onClose}>
      <div className="rl-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <button className="rl-btn solid" style={{ width: "100%", justifyContent: "center", background: isFinish ? undefined : "#A64B2A", borderColor: isFinish ? undefined : "#A64B2A" }} disabled={!canSave} onClick={handleSave}>
